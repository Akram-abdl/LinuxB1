...
# create lock file
touch /tmp/out/lock || { 
    error_createl = "script failed, cannot create lock file. Error for $file"
...
do 
    gzip  "$file" && echo "success compressing file $file" >> /tmp/out/logs.txt || {
        error_comp="script failed cannot compress files. Error for $file"
        echo $error_comp >> /tmp/out/logs.txt 
        echo $error_comp
    }
    mv "$file".gz /tmp/out && echo "success moving file $file to folder" >> /tmp/out/logs.txt || { 
        error_move = "script failed, cannot move files to /tmp/out. Error for $file"
...
rm /tmp/out/lock || { 
    error_deletel = "script failed, cannot delete lock file. Error for $file"
